...
- SPCS deployment is mandatory -- fails loudly if compute pools or image are unavailable
...
- Robust pipeline uses `AI_COMPLETE` with JSON schema `response_format` and `TRY_PARSE_JSON`
...
- SPCS image must be pushed before deployment -- see `spcs/push-image.sh`
